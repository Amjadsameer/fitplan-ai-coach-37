      ...
      {open && (
        <Modal onClose={() => setOpen(false)} title={t.createUser}>
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-2 rounded-xl border border-border bg-muted/30 p-3 text-sm"><input type="checkbox" checked={form.isAdmin} onChange={e => setForm({ ...form, isAdmin: e.target.checked })} className="h-4 w-4 accent-[var(--color-primary)]" /> <Shield className="h-3.5 w-3.5 text-primary" /> <span className="font-semibold">{lang === "ar" ? "إنشاء كـ Super Admin" : "Create as Super Admin"}</span></label>
